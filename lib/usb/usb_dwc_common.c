	...
	void (*callback)(usbd_device *usbd_dev, uint8_t ep))
{
	...
	if (ep == 0U) { /* For the default control endpoint */
		...
		REBASE(OTG_DOEPCTL(0U)) = OTG_DOEPCTL0_EPENA | OTG_DIEPCTL0_CNAK;
#else
...
#else
		for (size_t i = 0; i < usbd_dev->rxbcnt; i += 4U) {
