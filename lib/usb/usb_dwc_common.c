			...
			void (*callback) (usbd_device *usbd_dev, uint8_t ep))
{
	...
	if (ep == 0) { /* For the default control endpoint */
		...
		/* Configure OUT part. */
		usbd_dev->doeptsiz[0] = OTG_DOEPSIZ0_STUPCNT_1 | OTG_DOEPSIZ0_PKTCNT |
			(max_size & OTG_DOEPSIZ0_XFRSIZ_MASK);
	...
	if (addr & 0x80U) {
		...
		REBASE(OTG_DIEPCTL(ep)) = (max_size & OTG_DIEPCTLX_MPSIZ_MASK) | OTG_DIEPCTL0_SNAK |
			OTG_DIEPCTL0_USBAEP | (type << OTG_DIEPCTLX_EPTYP_SHIFT) | OTG_DIEPCTLX_SD0PID |
			(ep << OTG_DIEPCTLX_TXFNUM_SHIFT);
#else
		REBASE(OTG_DIEPTSIZ(ep)) = max_size & OTG_DIEPCTLX_MPSIZ_MASK;
		REBASE(OTG_DIEPCTL(ep)) |=
		    OTG_DIEPCTL0_EPENA | OTG_DIEPCTL0_SNAK | (type << OTG_DIEPCTLX_EPTYP_SHIFT) |
		    OTG_DIEPCTL0_USBAEP | OTG_DIEPCTLX_SD0PID |
		    (ep << OTG_DIEPCTLX_TXFNUM_SHIFT) | (max_size & OTG_DIEPCTLX_MPSIZ_MASK);
#endif
		...
		REBASE(OTG_DOEPCTL(ep)) = OTG_DOEPCTL0_EPENA | OTG_DIEPCTL0_CNAK | OTG_DOEPCTL0_USBAEP |
		    OTG_DOEPCTLX_SD0PID | (type << OTG_DIEPCTLX_EPTYP_SHIFT) | (max_size & OTG_DOEPCTLX_MPSIZ_MASK);
...
uint16_t dwc_ep_write_packet(usbd_device *const usbd_dev, const uint8_t addr,
			      const void *buf, const uint16_t len)
{
	const uint8_t ep = addr & 0x7FU;
