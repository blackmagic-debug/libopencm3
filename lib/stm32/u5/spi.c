	...
	const uint32_t data_size, const uint32_t lsbfirst, const uint32_t protocol)
{
	 ...
	 * This puts the peripheral in SPI master mode.
	 */
	SPI_CFG2(spi) &= ~(SPI_CFG2_CPOL | SPI_CFG2_CPHA | SPI_CFG2_LSBFIRST | SPI_CFG2_SP_MASK);
	SPI_CFG2(spi) |= SPI_CFG2_MASTER | cpol | cpha | lsbfirst | protocol;
